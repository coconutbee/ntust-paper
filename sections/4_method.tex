...
\begin{figure}[t]\centering
\includegraphics[width=0.6\linewidth]{figures/skeleton_templates.png}
...
quality). \revised{We set $\lambda{=}0.02$; App.~\ref{sec:appendix_lambda}
justifies this choice with a bracketing sweep against $\lambda\in\{0.005,0.1\}$
under the identical training budget.} From noise there is no leakage, so the only way to raise the cosine is
...
$K{\in}\{\mathrm{id},\mathrm{pose}\}$; $\sim$200 params, initialized to $0$) is
...
Verified skeleton selection, reference duplication, and the learned \gate{}
are, on the surface, three unrelated fixes: one picks a better pose control,
one repeats a token block, one trains $\sim$200 parameters. What justifies
treating them as one method rather than three tricks is that all three
target the same bottleneck---the single joint-attention budget introduced in
Sec.~\ref{sec:refgeometry}---without yet explaining \emph{why} that targeting
works or \emph{how much} of the lost identity it can recover. Chapter~\ref{sec:whyworks}
answers both questions directly, first verifying that identity really does
flow through that budget rather than through some other pathway
(Sec.~\ref{sec:refverify}), then quantifying exactly how the levers above
reallocate it (Sec.~\ref{sec:mechanism}).
