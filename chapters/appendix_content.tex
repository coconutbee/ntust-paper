...
the first value tried, we bracket it with a $3$-point sweep
$\lambda\in\{0.005,0.02,0.1\}$ ($5{\times}$ below and above), retraining from
scratch under the \emph{identical} $800$-step budget, cache, and $16$-image
held-out validation set used to report every number in this thesis, and
re-evaluating at the final ($800$-step) checkpoint of each run.

Table~\ref{tab:lambda_sweep} reports the result. \textbf{Pose is unaffected by
$\lambda$}: yaw accuracy is identical ($75.0\%$, $12/16$) at all three values,
exactly as the method predicts---the skeleton gain $\gamma_{\mathrm{skel}}$ is
\emph{frozen}, not regularized, so nothing about $\lambda$ can touch pose;
this sweep is an empirical check of that design guarantee, not merely a
restatement of it. The only axis $\lambda$ moves is identity, and it moves
\emph{non-monotonically}: the loosest setting ($\lambda{=}0.005$) reaches the
\emph{lowest} raw CSIM ($0.097$) of the three, not the highest, despite being
the least constrained---consistent with under-regularization leaving this
small, $200$-example-trained tensor less reliably guided during optimization,
not merely under-protected at convergence. $\lambda{=}0.02$ attains the best
raw CSIM ($0.138$) of the sweep, matched within noise by $\lambda{=}0.1$
($0.137$)---five-fold more regularization buys no further identity gain,
indicating the useful range has already plateaued by $\lambda{=}0.02$.
$\lambda{=}0.02$ is therefore the smallest value in the sweep that reaches the
plateau, giving the model the most room to depart from the \dupid{} baseline
while still landing in the region where additional freedom stops helping.
...
identical across all three $\lambda$ (the frozen skeleton gain protects pose by
construction); the loosest setting reaches the \emph{lowest} raw CSIM, and
$5\times$ more regularization than $0.02$ buys no further gain---$\lambda{=}0.02$
sits at the plateau's edge.}
...
even across the race categories that are present, extended with $4$ additional
verified reference identities (public figures Sundar Pichai, Satya Nadella,
Indra Nooyi, Anima Anandkumar; generated and scored under the identical
$1400$-generation pipeline configuration, kept in a separate pool so the
main $35$-identity benchmark is untouched) to populate a category that had
zero coverage. Race labels come from DeepFace, with a caveat worth stating
plainly: DeepFace's own classifier mislabeled $4$ of the $5$ verified
South-Asian public figures in this pool by race (three of the four newly added
identities, plus Malala Yousafzai already in the original $35$)---corrected
here to \emph{indian} based on verified public identity, not the classifier's
output, since named individuals admit independent ground truth that anonymous
crops do not. A fifth sourced candidate, mislabeled ``Arvind Krishna'' by a
Wikimedia Commons category page that conflates multiple unrelated people
sharing that name, turned out on inspection to be a different person entirely
and was dropped rather than corrected.

Table~\ref{tab:race} reports identity (raw and pose-aware AdaFace \csim{}) by
race. The four categories with $n{\geq}5$ identities---white, asian, black, and
the newly-populated indian---sit within a narrow band (raw \csim{}
$0.272$--$0.340$; pose-aware $0.125$--$0.195$), with no category showing a
systematic deficit; middle eastern ($n{=}2$) and latino hispanic ($n{=}1$, after
the race correction above moved Malala out of it) are flagged as
too small to support a claim either way rather than silently averaged in.
...
asian & 7 & 0.281 & 79.6 & 0.125 \\
black & 5 & 0.322 & 87.5 & 0.195 \\
indian & 5 & 0.340 & 75.5 & 0.128 \\
\midrule
middle eastern$^\dagger$ & 2 & 0.328 & 85.0 & 0.180 \\
latino hispanic$^\dagger$ & 1 & 0.520 & 82.5 & 0.258 \\
\bottomrule
\end{tabular}}
...
as Eq.~\ref{eq:posepenalty}; $40$ generations per identity). The four categories
with $n{\geq}5$ identities lie within a narrow band on both raw and pose-aware
CSIM; $^\dagger$flagged categories have too few identities ($n{<}5$) to support
