...
This appendix collects everything the main chapters summarize but do not have
room to show in full, in three groups. A.1--A.3 stress-test the evaluation
protocol itself---whether the pose-rectified ranking survives under a closer
look at which methods actually rotate (A.1), an alternative metric weighting
(A.2), and a fairer pose-control format for the ControlNet baselines (A.3).
A.4 justifies the backbone choice every other chapter assumes. A.5--A.6
supply the full causal and quantitative data behind Chapter 5's mechanistic
account of \emph{why} identity survives and \emph{why} reference order
matters. A.7--A.8 are reference material---the exact angle formulas and
identity-encoder definitions the main chapters cite by name but do not spell
out. \revised{A.9--A.11 close three further robustness questions: whether the
$\gamma$-gate's regularization strength $\lambda$ is justified rather than
arbitrary (A.9), whether identity preservation is stable across the reference
identity's race (A.10), and whether additional prompt content beyond pose
costs identity or pose (A.11).}

\section{Per-method pose distributions}
\label{sec:appendix_thresh}
We start with the evaluation protocol's most basic question: which methods
actually cross the turn and over-the-shoulder boundaries, as opposed to only
claiming to? The pose-angle definitions (head yaw, pitch, body yaw, and the head-to-torso yaw
...
all eight pose cells is Table~\ref{tab:prompts}. Here we report the full
per-method head-yaw, pitch, and $\Delta\theta_y$ histograms
(Fig.~\ref{fig:methoddist}) behind that question.
...
A.1 established which methods actually rotate; we now check that this is not
an artifact of how the identity metric itself is weighted. Our pose-aware
identity (Sec.~\ref{sec:metrics}) penalizes by yaw alone,
...
\label{sec:exp_skel}
A.2 confirmed the identity metric's own design is not deciding the ranking;
we turn now to a different confound in the \emph{comparison} itself. Every
ControlNet baseline in the main results was fed our SAM-3D skeleton, which is
foreign to their own training format---so a natural question is whether the
rotation gap is merely an artifact of that mismatch: would any model, given a
pose control \emph{in the format it expects}, rotate as well?
...
A.3 showed that even a favorably-formatted external ControlNet still trails
Rotate-ID's in-context conditioning---so the comparison is fair, but it begs a
different question: why condition FLUX.2-klein specifically, rather than any
other backbone? This section justifies that choice on both axes that matter:
rotation quality and the compute budget the rest of the thesis depends on.
...
With FLUX.2-klein justified as the base, we turn from practical constraints to
a mechanistic one: does identity actually survive its frozen VAE and joint
attention, or could the causal story of Sec.~\ref{sec:refverify}'s three
verified links (the VAE preserves identity-relevant detail; the raw
pre-attention reference tokens do not yet encode it legibly; the joint
attention reads it from the reference's \emph{content}, not its sequence
position) be an artifact of how those experiments were scored? This section
gives the complete data behind each link.
...
A.5 showed identity tracks the reference's \emph{content}, not the identity
slot's position in the sequence---so it is natural to ask whether \emph{list
order} (which block comes first) should matter at all. It does, but not by
contradicting A.5: order does not change \emph{which} content the model reads,
only \emph{how much} attention that content receives, because the joint
attention's RoPE coordinates are order-dependent even though its content
read-out is not. Sec.~\ref{sec:refgeometry} predicts that \emph{order} is a
real variable on exactly this basis: the
...
A.1--A.6 verified the evaluation protocol and the method's causal mechanism;
the remaining two sections are reference material the earlier chapters cite
by name without room to spell out. We start with the geometry behind every
yaw/pitch/over-the-shoulder number reported so far: this section gives the
full formulas summarized in Sec.~\ref{sec:metrics}.
...
A.7 gave the formulas behind every \emph{pose} number in this thesis; this
closing section documents the encoder behind every \emph{identity} number,
and gathers the full evidence that its profile read-out is trustworthy.
...
\revised{
\paragraph{Relating the pose-aware discount to PAEs.} The pose-aware identity
$\widetilde{\mathrm{CSIM}}(k){=}\mathrm{CSIM}\cdot e^{-kP_e}$
(Eq.~\ref{eq:posepenalty}) and PAEs attack the \emph{same} frontal-bias problem
by two independent routes, and neither is validated against a ground-truth
same-pose photograph---no such photograph exists for a synthesized identity, and
we do not re-synthesize the reference at the achieved pose to manufacture one
(a route we considered and set aside, since it would substitute one generative
model's rendering for another rather than supply independent ground truth).
The two corrections instead check each other. PAEs corrects the \emph{encoder}:
it swaps in a pose-appropriate recognizer so CSIM is read fairly even when a
hard angle \emph{was} achieved, addressing the case a method rotates correctly
but AdaFace still under-credits it. The pose-aware discount corrects the
\emph{score}: it penalizes CSIM whenever the requested pose was \emph{not}
achieved, regardless of which encoder measured it, addressing the case a
method never rotates and free-rides on AdaFace's frontal bias. These are
different failure modes with no logical reason to agree---yet applying the
identical discount to both encoders produces the same verdict: at $k{=}4$,
\textbf{Rotate-ID} leads every tuning-free baseline on \emph{both} the AdaFace-
and the PAEs-based pose-aware score (Table~\ref{tab:posepenalized}), and the
un-discounted PAEs read-out already recovers most of the apparent profile loss
before any discount is applied (the recovery documented above). Two
independently-motivated corrections to two different failure modes converging
on the same ranking is the evidence this thesis offers for the discount's
validity, in place of a same-pose ground-truth photograph that does not exist.
}

\revised{
\section{Regularization strength: choosing $\lambda$ for the learned $\gamma$-gate}
\label{sec:appendix_lambda}
Sec.~\ref{sec:learned} trains the $\sim$200-parameter allocator with
$\mathcal{L}=(m-\cos)_{+}+\lambda\,\overline{(\log\gamma)^2}$
(Eq.~\ref{eq:gateloss}, $\lambda{=}0.02$): the regularizer is an $L_2$ penalty
in \emph{log}-space, so it is a ridge/shrinkage prior pulling every
$(\text{step},\text{layer},\text{kind})$ gain back toward $\log\gamma{=}0$,
i.e.\ $\gamma{=}1$, the causally-verified \dupid{} winner baseline
(Sec.~\ref{sec:whyworks})---not an arbitrary weight but a knob that trades
how far training is allowed to depart from that baseline against how much
identity gain it is permitted to chase. To check $\lambda{=}0.02$ is not simply
the first value tried, we bracket it with a $4$-point sweep
$\lambda\in\{0.005,0.02,0.1,0.5\}$ (a $4\times$ and $25\times$ increase, plus a
$4\times$ decrease), retraining from scratch under the \emph{identical}
$800$-step budget, cache, and $16$-image held-out validation set used to report
every number in this thesis, and re-evaluating at the final ($800$-step)
checkpoint of each run.

Table~\ref{tab:lambda_sweep} and Fig.~\ref{fig:lambda_sweep} report the result.
\textbf{Pose is unaffected by $\lambda$}: yaw accuracy is identical ($75.0\%$,
$12/16$) at \emph{all four} values, exactly as the method predicts---the
skeleton gain $\gamma_{\mathrm{skel}}$ is \emph{frozen}, not regularized, so
nothing about $\lambda$ can touch pose; this sweep is an empirical check of
that design guarantee, not merely a restatement of it. The only axis $\lambda$
moves is identity, and it traces a clear inverted-U: the loosest setting
($\lambda{=}0.005$) under-shoots ($0.097$ raw CSIM), $\lambda{=}0.02$ and
$\lambda{=}0.1$ sit on a plateau at the top ($0.138$/$0.137$), and pushing
$25\times$ higher to $\lambda{=}0.5$ collapses \emph{below even the loosest
setting} ($0.076$)---within reach of the untrained ($\gamma{=}1$) baseline's
$0.024$ pose-aware floor, i.e.\ the regularizer has begun to cancel out the
training signal entirely. $\lambda{=}0.02$ is the smallest value in the sweep
that reaches the plateau, giving the model the most room to depart from the
\dupid{} baseline while still avoiding both failure mode: too little
regularization leaves the small, $200$-example-trained tensor under-guided
during optimization, and too much drags it back toward the untrained baseline.
...
\begin{tabular}{lcccc}
\toprule
$\lambda$ & raw CSIM (Ada) & Yaw acc & pose-aware $\widetilde{\mathrm{CSIM}}$ ($k{=}4$) \\
\midrule
$0.005$ & $0.097$ & $75.0$ & $0.036$ \\
\textbf{$0.02$ (used)} & $\mathbf{0.138}$ & $75.0$ & $\mathbf{0.051}$ \\
$0.1$ & $0.137$ & $75.0$ & $0.050$ \\
$0.5$ & $0.076$ & $75.0$ & $0.028$ \\
\midrule
\emph{untrained ($\gamma{=}1$, $\lambda\to\infty$ limit)} & $0.107$ & -- & $0.024$ \\
\bottomrule
\end{tabular}}
\caption[Regularization-strength sweep for the learned $\gamma$-gate]{\textbf{Regularization-strength sweep for the learned $\gamma$-gate}
($16$-image held-out validation set, identical $800$-step training budget per
$\lambda$; pose-aware $\widetilde{\mathrm{CSIM}}{=}\mathrm{CSIM}\cdot e^{-4P_e}$,
$P_e{=}1{-}\mathrm{Acc_{yaw}}$, as Eq.~\ref{eq:posepenalty}). Yaw accuracy is
identical across all four $\lambda$ (the frozen skeleton gain protects pose by
construction). Identity traces an inverted-U: $\lambda{=}0.02$ sits at the top
of the plateau, while $25\times$ more regularization ($\lambda{=}0.5$) collapses
\emph{past} the loosest setting, approaching the untrained floor.}
\label{tab:lambda_sweep}
\end{table}

\begin{figure}[t]\centering
\includegraphics[width=0.72\linewidth]{figures/lambda_sweep_curve.png}
\caption[Regularization-strength sweep, visualized]{\textbf{Regularization-strength sweep, visualized} (same data as
Table~\ref{tab:lambda_sweep}, log-$\lambda$ axis). Both raw and pose-aware CSIM
trace the same inverted-U; the dashed line is the untrained baseline that
$\lambda\to\infty$ converges to. $\lambda{=}0.02$ (circled) sits at the plateau
where increasing $\lambda$ further stops helping and decreasing it starts
hurting, in both directions.}
\label{fig:lambda_sweep}
\end{figure}
}

\revised{
\section{Identity stability across the reference identity's race}
\label{sec:appendix_race}
The $35$ reference identities (Fig.~\ref{fig:idgallery}) were not curated for
race balance; this section checks whether Rotate-ID's identity preservation is
even across race, extended with $34$ additional verified reference identities
(public figures spanning asian, black, middle eastern, latino hispanic, and
indian; generated and scored under the identical $1400$-generation pipeline
configuration, kept in a separate pool so the main $35$-identity benchmark is
untouched) so that \emph{every} category reaches at least $n{=}10$ identities,
including two categories (middle eastern, latino hispanic) that started with
only $1$--$2$.

Sourcing at this scale surfaced a finding worth stating plainly: DeepFace's own
race classifier mislabeled $17$ of the $30$ newly sourced public figures
($57\%$)---not only the previously-noted South-Asian bias (e.g.\ Priyanka
Chopra, Shah Rukh Khan, Aishwarya Rai all read as \emph{white}), but broadly
across categories (Barack Obama read as \emph{latino hispanic}; Jennifer Lopez
as \emph{asian}; Jordan's Queen Rania and Egyptian-American news anchor Hoda
Kotb both read as \emph{white}). Every label in this section is therefore the
verified public identity of the named individual, \emph{not} DeepFace's raw
output, which independent ground truth makes possible only for named figures.
This also means the classifier's reliability on the original $35$ anonymous
reference crops---which admit no such independent check---cannot be
established one way or the other; the race labels for that portion of the
benchmark should be read as approximate. (One additional sourcing error is
also worth disclosing: a Wikimedia Commons category mislabeled ``Arvind
Krishna'' conflated multiple unrelated people sharing that name; the resulting
photo was a different person entirely and was dropped rather than corrected.)

Table~\ref{tab:race} and Fig.~\ref{fig:race} report identity (raw and
pose-aware AdaFace \csim{}) by race, now with every category at $n{\geq}10$.
All six sit within a narrow band (raw \csim{} $0.242$--$0.302$; pose-aware
$0.099$--$0.167$), with no category showing a systematic deficit; asian is the
lowest on both measures and black the highest, but the spread is modest next
to the gap this thesis reports between methods (Table~\ref{tab:posepenalized}).
...
\begin{tabular}{lcccc}
\toprule
Race & $n$ identities & raw CSIM (Ada) & Yaw acc & pose-aware ($k{=}4$) \\
\midrule
white & 19 & 0.272 & 82.2 & 0.133 \\
black & 10 & 0.302 & 85.2 & 0.167 \\
indian & 10 & 0.301 & 80.2 & 0.137 \\
latino hispanic & 10 & 0.271 & 82.0 & 0.132 \\
middle eastern & 10 & 0.275 & 81.8 & 0.132 \\
asian & 10 & 0.242 & 77.5 & 0.099 \\
\bottomrule
\end{tabular}}
\caption[Identity preservation by reference race]{\textbf{Identity preservation by reference identity's race} (pose-aware
$\widetilde{\mathrm{CSIM}}{=}\mathrm{CSIM}\cdot e^{-4P_e}$, $P_e{=}1{-}\mathrm{Acc_{yaw}}$,
as Eq.~\ref{eq:posepenalty}; $40$ generations per identity). Every category now
has $n{\geq}10$ identities; all six lie within a narrow band on both raw and
pose-aware CSIM, with no systematic deficit in any category.}
\label{tab:race}
\end{table}

\begin{figure}[t]\centering
\includegraphics[width=0.85\linewidth]{figures/race_csim_by_category.png}
\caption[Identity preservation by race, raw vs.\ pose-aware]{\textbf{Identity preservation by race, raw vs.\ pose-aware} (same
data as Table~\ref{tab:race}, all categories $n{\geq}10$). The six categories
cluster within a modest range on both raw and pose-aware CSIM; asian is
lowest, black highest, but no category is a systematic outlier.}
\label{fig:race}
\end{figure}
}

\revised{
\section{Additional prompt conditions: does richer conditioning cost identity or pose?}
\label{sec:appendix_condition1}
Every prompt in the main benchmark (Table~\ref{tab:prompts}) already varies
demographic and scene attributes; this section asks a narrower question raised
independently of that design---whether \emph{explicitly} adding a facial
expression or a facial accessory to the prompt trades away identity, pose, or
neither. We select $100$ (identity, prompt) pairs stratified across all $40$
canonical prompts from the main $1400$-generation benchmark and, for each,
generate three variants that append a clause to the \emph{exact} original
prompt text---an expression (rotated through five variants: smiling,
surprised, confused, calm, sad), a facial accessory (rotated through five
variants: glasses, sunglasses, a hat, earrings, a scarf), or
both---while reusing the identical reference image, skeleton control, seed, and
$\gamma$-gate configuration as that pair's already-scored baseline row, so the
$300$ new generations are a paired comparison against zero new baseline
generation.

Table~\ref{tab:condition1} reports the result. \textbf{Identity is largely
unaffected}: an expression clause leaves raw CSIM essentially unchanged
($0.292{\to}0.292$); the accessory conditions do show a CSIM drop ($\to\!0.23$),
plausibly because glasses/sunglasses/a hat genuinely occlude the face region
AdaFace reads, rather than the model losing the identity. \textbf{Pose is the
axis that pays}: yaw accuracy falls from $80.0\%$ to $65$--$69\%$ under
\emph{every} added condition, by a comparable margin regardless of whether the
addition is an expression or an accessory---consistent with the
attention-budget account of this thesis (Sec.~\ref{sec:lim-scale}): additional
semantic content the prompt must render competes for the same finite
conditioning capacity that otherwise goes toward realizing the requested pose.
...
\toprule
Condition & $n$ & face det.\ & raw CSIM (Ada) & Yaw acc & pose-aware ($k{=}4$) \\
\midrule
baseline (no addition) & 100 & 0.90 & 0.292 & 80.0 & 0.131 \\
+\,expression & 100 & 0.94 & 0.292 & 65.0 & 0.072 \\
+\,accessory & 100 & 0.88 & 0.233 & 69.0 & 0.068 \\
+\,both & 100 & 0.92 & 0.238 & 66.0 & 0.061 \\
\bottomrule
\end{tabular}}
\caption[Additional prompt conditions vs.\ identity and pose]{\textbf{Additional prompt conditions vs.\ identity and pose} ($100$
paired identity/prompt pairs $\times$ each condition; pose-aware
$\widetilde{\mathrm{CSIM}}{=}\mathrm{CSIM}\cdot e^{-4P_e}$, $P_e{=}1{-}\mathrm{Acc_{yaw}}$,
as Eq.~\ref{eq:posepenalty}). Identity is close to unaffected by an expression
clause and drops modestly under an accessory clause (plausibly genuine visual
occlusion); pose accuracy drops by a comparable margin under \emph{any} added
condition, identifying pose---not identity---as what additional conditioning
competes with.}
\label{tab:condition1}
\end{table}
}
