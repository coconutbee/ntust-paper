...
\label{sec:exp_id}
The pose-aware reading (Table~\ref{tab:posepenalized}) already shows that, once
pose is accounted for, Rotate-ID's identity overtakes the frontal-collapsing
baselines. Two observations explain why. First, the pose-robust \emph{PAEs} score
is consistently higher than overall AdaFace (Rotate-ID $0.377$ vs.\ $0.297$;
Table~\ref{tab:sota_pose}), confirming that much of the apparent profile identity
loss is a \emph{recognition-metric artifact}---the angle-matched expert still finds
the identity, and a \emph{single} cross-pose encoder ($E_{fs}$ or $E_{fp}$) applied
to all images matches the routed PAEs to within $\sim\!0.04$, so the recovered
identity reflects the pose-adapted encoders, not the bin-selection rule. Second,
Rotate-ID builds this identity from the bare backbone with no prompt edit:
Table~\ref{tab:id_bins} isolates the contribution of each component, and we
return to it as the formal component ablation in Sec.~\ref{sec:exp_ablate}.

\noindent\textbf{Takeaway.} \emph{Much of the profile identity drop is the
recognizer's artifact (PAEs $>$ AdaFace everywhere); the learned gate recovers
the rest purely by reallocating attention to the reference, with no prompt
edit.}
...
The baselines earn their high aggregate identity on near-frontal outputs
(Table~\ref{tab:sota_pose}). To summarize the identity--rotation trade-off in a
\emph{single} number we use the \emph{pose-aware identity}
$\widetilde{\mathrm{CSIM}}(k)=\mathrm{CSIM}\cdot e^{-k P_e}$ (Sec.~\ref{sec:metrics},
...
larger $k$ punishing unrealized rotation more---the formula form of our central
claim, \emph{identity only counts at the pose actually requested}.
...
This section ablates every mechanism-level design decision of
Secs.~\ref{sec:refgeometry}--\ref{sec:learned}: the presence of each component,
the duplication \emph{count}, the reference \emph{order}, and the learned gate,
each scored with the pose-rectified metrics of Sec.~\ref{sec:metrics}.
...
\begin{figure}[t]\centering
\includegraphics[width=\textwidth]{figures/mech_visual_settings_progression.png}
\caption[Every setting, one prompt and seed]{\textbf{Every setting, one prompt and seed} (full frames; AdaFace \csim{}
and pose verdict under each image)---the multi-identity view of
Fig.~\ref{fig:idemergence}, pushed on a single case through the full
duplication range to the copy-paste endpoint. A single reference loses the identity
entirely (wrong person); duplication recovers it by $N{=}3$; the \gate{} (green
frame) refines it further; and over-duplication ($N{\ge}8$) collapses into
...
\begin{figure}[t]\centering
\includegraphics[width=\linewidth]{figures/idprompt_noskel.png}
\caption[FLUX.2 with the identity image and prompt \emph{only} (no skeleton)]{\textbf{FLUX.2 with the identity image and prompt \emph{only} (no
skeleton).} Each generated cell is annotated with its AdaFace CSIM to the
reference (green $\geq\!0.25$, red below) and its measured yaw. Without structural
control the backbone does rotate the head, but renders the prompt's nominal
person, not the reference---the \emph{wrong} identity (CSIM ${\approx}\,0.13$); the full
Rotate-ID rotates \emph{and} preserves the reference identity. This is the naive
